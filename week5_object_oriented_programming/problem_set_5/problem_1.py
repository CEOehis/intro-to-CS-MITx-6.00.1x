...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 10:07:21 2019
...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(lower)):
            try:
                shiftedDict[lower[i]] = lower[i + shift]
            except IndexError:
                shiftedDict[lower[i]] = lower[shift - (len(lower) - i)]
        upper = string.ascii_uppercase
        for i in range(len(upper)):
            try:
                shiftedDict[upper[i]] = upper[i + shift]
            except IndexError:
                shiftedDict[upper[i]] = upper[shift - (len(upper) - i)]
        return shiftedDict
